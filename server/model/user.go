...
type User struct {
	BaseModel
	Nickname     string         `gorm:"type:varchar(64);not null;index:idx_user_nickname;default:'';comment:昵称" json:"nickname"`
	Username     string         `gorm:"type:varchar(64);not null;uniqueIndex:uk_user_username;comment:用户名" json:"username"`
	Mobile       string         `gorm:"type:varchar(20);not null;uniqueIndex:uk_user_mobile;comment:手机号" json:"mobile"`
	HideMobile   uint           `gorm:"type:tinyint unsigned;not null;default:0;comment:是否隐藏手机号（0：否，1：是）" json:"hide_mobile"`
	Email        string         `gorm:"type:varchar(100);not null;uniqueIndex:uk_user_email;comment:邮箱" json:"email"`
	...
	LastLoginAt  *carbon.Carbon `gorm:"type:datetime;index:idx_user_last_login_at;comment:最后登录时间" json:"last_login_at"`
	Status       uint           `gorm:"type:tinyint unsigned;not null;default:0;comment:用户状态（0：禁用，1：启用）" json:"status"`
	ExpireAt     *carbon.Carbon `gorm:"type:datetime;index:idx_user_expire_at;comment:过期时间" json:"expire_at" copier:"-"`
	SystemRoleId uint           `gorm:"not null;index:idx_user_role_id;comment:角色ID" json:"system_role_id"`
